-- Migration 007: Auth trigger for auto-creating public.users on signup
-- When a new auth.users row is created, this trigger auto-inserts
-- a matching public.users row using metadata passed during createUser().
...
-- Attach trigger (drop first if exists)
